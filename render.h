...
int main(int argc, char **argv){
    ...
    if(!event_queue) {
        ...
        al_destroy_bitmap(bouncer); al_destroy_display(display);
        al_destroy_timer(timer);
        return -1;
    }
    ...
    while(1) {
        ...
        if(ev.type == ALLEGRO_EVENT_TIMER) {
            if(bouncer_x < 0 || bouncer_x > SCREEN_W - BOUNCER_SIZE) { bouncer_dx = -bouncer_dx; }
            if(bouncer_y < 0 || bouncer_y > SCREEN_H - BOUNCER_SIZE) { bouncer_dy = -bouncer_dy; }
        ...
        if(redraw && al_is_event_queue_empty(event_queue)) {
            redraw = false; al_clear_to_color(al_map_rgb(0,0,0));
